...
<div id="bp-switch-container" class="mt-[5px] px-[50px]">
  ...
  <div class="flex flex-row justify-center items-center w-[100%] mt-[10px] bg-primaryBg px-[50px]">
    <button class="bg-[#2081E2] text-[26px] lg:text-[16px] font-semibold border-none text-white py-4 w-[60%] rounded-[3px]" onclick="switchNetwork(${globalThis.requiredTokenDetail?.chainDetails?.chain_id.toString(16)}, ${globalThis.requiredTokenDetail?.chainDetails?.backendName}); closePopup();}">Switch</button>
  </div>
