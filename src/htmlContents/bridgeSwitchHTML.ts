...
export const bridgeSwitchHTML = `'<div class="rounded-[30px] z-50 m-auto bg-primaryBg w-[90%] lg:w-[30%] justify-between items-center" id="bridge-popup-css">'+
  '<div class=" bg-primaryBg] rounded-t-[30px] p-5 flex flex-col justify-start items-center w-full">'+
    ...
    '</div>'+
    '<div id="bp-heading">'+
      '<h2 class="font-semibold text-[28px] my-5 lg:my-10 text-primaryText text-center">Switch to '+ fetchEthereumChainData("0x" + chainId.toString(16)).chainName +' for this exchange</h2>'+
    '</div>'+
    '<div class="flex items-start justify-evenly w-[95%] my-[50px]">'+
      '<div id="bp-switch-chain-container">'+
        '<img src="https://public.cypherd.io/icons/logos/' + fetchChainDetails(currentChainId).backendName.toLowerCase() + '.png" class="w-[42px] h-[42px]" alt="' + chainName + ' logo">'+
      ...
      '</div>'+
      '<div id="bp-switch-chain-container">'+
        '<img src="https://public.cypherd.io/icons/logos/' + chainName.toLowerCase() + '.png" class="w-[42px] h-[42px]" alt="' + chainName + ' logo">'+
  ...
  '<div class="bg-primaryBg py-5 lg:py-10 w-full flex items-center justify-center rounded-b-[30px]">'+
    '<button class="bg-[#2081E2] py-4 w-[60%] rounded-lg text-white text-[16px] font-semibold" onclick="navigateAfterSwitch(' + "'0x" + chainId.toString(16) + "'" + ', ' + "'" +  chainName + "'" + ')">Switch</button>'+
  '</div>'+
