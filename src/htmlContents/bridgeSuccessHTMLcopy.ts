export const bridgeSuccessHTMLCopy = `
<div id="bridge-popup-css" class="rounded-[30px] pt-[30px] w-[90%] lg:w-[35%] justify-evenly bg-primaryBg">
  ...
  <p class="text-center text-[36px] lg:text-[18px] text-primaryText mt-[5px] px-10 lg:px-0">You have '+ parseFloat(globalThis.bridgeQuote ? globalThis?.bridgeQuote?.transferAmount : globalThis.swapQuoteData?.toToken?.amount)?.toFixed(6) + ' ' + globalThis?.requiredTokenDetail?.symbol + ' added in ' + globalThis?.requiredTokenDetail?.chainDetails?.backendName + '. You can now use dapp.</p>

  <div class=" flex flex-row justify-center px-[50px] items-center w-[100%] mt-[50px] bg-primaryBg">
    <button class="bg-[#2081E2] text-[32px] lg:text-[16px] font-semibold border-none text-white py-4 w-[60%] rounded-[3px]" onclick="closePopup(); globalThis.callBack(true)">OK</button>
  </div>
