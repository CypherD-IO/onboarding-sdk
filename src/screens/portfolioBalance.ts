...
export const portfolioBalance = (bridgeableTokensList = globalThis.bridgeableTokensList, parentElement = document.getElementById("cyd-popup-background")) => {
  ...
  const tokensAvailableList = bridgeableTokensList.map((tokenDetail: any) => (
    `
        <tr class='odd:bg-stripedTableBg h-[75px] cyd-portfolio-token-detail'>
          ...
          <td>
            <p class='text-[10px] lg:text-[14px] text-primaryText font-semibold'>$ ${(_.get(tokenDetail, ['actualBalance']) * _.get(tokenDetail, ['price'])).toFixed(2)}</p>
          </td>
          <td>
  ...
  const tokenListContainer = `
    <div class='w-[100%] max-h-[60%] flex flex-col justify-center items-center'>
      ...
      <div class='cyd-tokens-available-flex-box w-[90%] lg:w-[80%] mt-2 lg:mt-0'>
  ...
  const portfolioBalanceHTML = `
    <div
      ...
      class="flex flex-col items-center cyd-maximize-onclick justify-between max-h-[85%] rounded-[30px] bg-primaryBg w-11/12 lg:w-3/5"
    >
