...
export const bridgeInput = (parentElement = document.getElementById("cyd-popup-background"), previousPage = portfolioBalance) => {
  const {
    requiredTokenDetail,
        ...
        <div class="bg-primaryBg border border-[#E4E4E4] p-2 flex rounded-2xl w-[90%] lg:w-[75%] mt-6">
          ...
          <div class="w-full ml-3 flex flex-col h-[100%]">
            <div id="cyd-bp-balance-detail-usd" class="flex justify-between h-[25px]">
              ...
              <p class="text-primaryText text-[18px] font-semibold" id="cyd-bp-balance-detail-usd-value">$${(actualBalance * price).toFixed(2)}</p>
            </div>
            <div id="cyd-bp-balance-detail-token" class="flex justify-between h-[25px]">
              <p class="text-[#929292] text-[16px] font-normal">${symbol}</p>
              <p class="text-[#929292] text-[16px] font-normal" id="cyd-bp-balance-detail-token-value">${parseFloat(actualBalance).toFixed(6)}</p>
            </div>
          </div>
        </div>
      </div>
