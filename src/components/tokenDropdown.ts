...
export const tokenDropdown = (options: any, id: string, disabled = false) => {
  const tokenDropdownHTML =
    ...
    `' class='cyd-dropdown-button text-[16px] text-primaryText justify-between rounded-[10px] cyd-md:rounded-[15px] bg-primaryBg relative flex items-center border border-primaryBg p-[10px] cyd-md:p-[14px] hover:border-borderColor w-full'>
      ...
      0,
      "logoUrl",
    ])} class='h-5 w-5 cyd-md:h-6 cyd-md:w-6 mx-[5px] cyd-md:mx-[20px] rounded-full w-4/12' />
        ...
        (option: string) =>
          ...
          `' class='cyd-dropdown-option flex justify-between items-center rounded-[15px] p-[2px] cyd-md:p-[5px] mt-[10px] hover:bg-soapstoneBg'>
                  <div class="flex flex-row items-center justify-center w-90%">
                    ...
                    <div class='text-[14px] cyd-md:text-[16px] text-primaryText w-8/12'>
                      ${_.get(option, ["name"])}
                    </div>
                  </div>
                  <div>
                    <div class='text-primaryText text-[12px] cyd-md:text-[14px]'>
                      $ ${(
                        _.get(option, ["actualBalance"]) *
                        _.get(option, ["price"])
                      ).toFixed(2)}
                    </div>
                    <div class='text-primaryText text-[10px] cyd-md:text-[12px]'>
